...
class MiniHeatmap {
  ...
  update(values) {
    ...
    var overlay = this.heatmap.append("svg").attr('class', 'overlay').attr('x', 0).attr('y', 0);
    var relativeWidthScale = d3.scaleLinear().domain([0, 752]).range([0, this.width]);
    var relativeHeightScale = d3.scaleLinear().domain([0, 580]).range([0, this.height]);
    var elementColor = d3.rgb(getElementColor(this.data.type.abbreviation)(1)); // Get coordinates as data arrays

    var xCoords = values.map(value => {
      return relativeWidthScale(parseFloat(value.coords.x));
    }).sort();
    var yCoords = values.map(value => {
      return relativeHeightScale(parseFloat(value.coords.y));
    }).sort();
    ...
    var verticalCenter = (yCoords[0] + yCoords[yCoords.length - 1]) / 2; // Heatmap Color Range

    var getColor = d3.scaleSequential().interpolator(getElementColor(this.data.type.abbreviation)).domain([0, max.rel]); // Data Points
    ...
    overlay.selectAll('rect').data(values).enter().append("rect").attr("class", "heatmap-data").attr("x", function (d) {
      return relativeWidthScale(parseFloat(d.coords.x));
    }).attr("y", function (d) {
      return relativeHeightScale(parseFloat(d.coords.y));
    }).attr("width", this.valueWidth).attr("height", this.valueWidth).style("fill", function (d) {
      return getColor(d.relative);
    }).style("opacity", this.opacityLevel).on("mouseover", function (d) {
      // Highlight data point
      d3.select(this).style("fill", "red").style('opacity', 1); // .style("stroke", elementColor)
